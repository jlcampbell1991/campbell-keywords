...
object User extends Model with Queries {
  ...
  def find[F[_]: Sync](id: UserId)(implicit XA: Transactor[F]): F[User] =
    sql"""select * from campbell_keywords_user where id = ${id.toString}"""
      .query[User]
      .unique
      .transact(XA)

  def find[F[_]: Sync](name: String)(implicit XA: Transactor[F]): F[User] =
    sql"""select * from campbell_keywords_user where name = ${name}"""
      .query[User]
      .unique
      .transact(XA)
  ...
  def create[F[_]: Sync](user: User)(implicit XA: Transactor[F]): F[User] = {
    sql"""
    insert into campbell_keywords_user (name, password, id)
    values
    (
      ${user.name},
      ${user.password},
  ...
  def update[F[_]: Sync](user: User): Update0 =
    sql"""
      update campbell_keywords_user set
        name = ${user.name},
  ...
  def destroy[F[_]: Sync](user: User): Update0 =
    sql"""delete from campbell_keywords_user where id = ${user.id}""".update
